...
`$1D08`|`$1DFF`| This is where the IRQ controller is uploaded at game reset and is responsible for controlling all S-CPU IRQs received. Call `$1D08` to activate IRQ-as-NMI system on the next IRQ call, if enabled.
